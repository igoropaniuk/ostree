...
test_read_xattrs (void)
{
  ...
  g_autofree char *tmpd_tmpl = g_build_filename (test_tmpdir, "ostree-xattrs-test.XXXXXX", NULL);
    ...
    if (r != 0)
      {
        g_autofree gchar *message = g_strdup_printf (
            "Unable to set extended attributes in %s: %s", test_tmpdir, g_strerror (errno));
        g_test_skip (message);
        return;
      }
